...
# --- Usage-based multi-key pool for GROQ API keys (no long cooldown) ---
class SmartGroqAPIKeyPool:
    def __init__(self, keys, usage_window=60):
        self.keys = keys
        self.lock = threading.Lock()
        self.usage_window = usage_window
        self.usage = {k: collections.deque() for k in keys}
        self.cooling = {}
        self.last_used = {k: 0 for k in keys}

    def _prune_usage(self, now):
        for key in self.keys:
            while self.usage[key] and self.usage[key][0] < now - self.usage_window:
    ...
    def get_key(self):
        with self.lock:
            ...
            available_keys = [k for k in self.keys if self.cooling.get(k, 0) <= now]
            if not available_keys:
                soonest_key = min(self.cooling, key=lambda k: self.cooling[k])
                wait = max(0, self.cooling[soonest_key] - now)
                logger.warning(f"[kg290] All API keys cooling down, waiting {wait:.1f}s...")
                time.sleep(wait)
                return soonest_key
            least_used = min(available_keys, key=lambda k: (len(self.usage[k]), self.last_used[k]))
            self.usage[least_used].append(now)
            self.last_used[least_used] = now
            return least_used

    def mark_rate_limited(self, key, retry_after=None):
        with self.lock:
            cooldown = float(retry_after) if retry_after else 2
            self.cooling[key] = time.time() + cooldown
...
groq_pool = SmartGroqAPIKeyPool(groq_keys)
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if hasattr(response, "status_code") and response.status_code == 429:
                    ...
                    logger.warning(f"[kg290] Rate limited for API key, marking cooling for {retry_after or 2}s and retrying...")
                    groq_pool.mark_rate_limited(api_key, retry_after)
                    continue
...
async def process_one_question(question, pdf_text):
    attempts = 3
    for attempt in range(attempts):
        result = await cloud_only_analysis(question, pdf_text)
        if result.get("success", False):
            return result.get("answer", "Unable to determine from the policy document.")
        logger.warning(f"[kg290] Attempt {attempt+1}: Failed to get answer, retrying...")
        await asyncio.sleep(1.0)
    return f"Analysis failed: Could not process after {attempts} attempts."
...
async def hackathon_endpoint(
    ...
    try:
        ...
        # True parallel: all questions are dispatched as async tasks
        tasks = [process_one_question(q, pdf_text) for q in request.questions]
