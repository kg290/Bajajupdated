...
async def cloud_only_analysis(query: str, pdf_text: str, api_key_used=None) -> Dict[str, Any]:
...
Analyze this question based on the policy document and provide your response in the specified JSON format."""
        ...
        for attempt in range(max_attempts):
            try:
                ...
                response.raise_for_status()
                # Try to get tokens_used if present
                try:
        ...
        try:
            ...
            return {
                ...
                "api_key_used": api_key[-5:]  # Only log part of the key for debug
            }
            ...
            for attempt in range(3):  # Increase attempts to use all keys
                ...
                if result.get("success", False):
                    answer = {
                        ...
                        "confidence": result.get("confidence", "medium (0.6)"),
                        "confidence_score": result.get("confidence_score", 0.6),
                        "supporting_clauses": result.get("supporting_clauses", []),
                        "reasoning": result.get("reasoning", ""),
                        ...
                        "tokens_used": result.get("tokens_used"),
                        "time_used_seconds": result.get("time_used_seconds"),
                        # "api_key_used": result.get("api_key_used"), # (optional, debug only)
                        "question_index": i + 1,
                        "total_questions": len(request.questions)
                    }
                    logger.info(
                        f"[kg290] [Q{i+1}/{len(request.questions)}] confidence: {answer['confidence']} | "
                        f"time_used: {answer['time_used_seconds']}s | tokens_used: {answer['tokens_used']} | key: ...{result.get('api_key_used')}"
                    )
                ...
                answer = {
                    "answer": f"Analysis failed: Could not process after 3 attempts.",
                    "confidence": "low (0.3)",
                    "confidence_score": 0.3,
                    "supporting_clauses": [],
                    "reasoning": "",
