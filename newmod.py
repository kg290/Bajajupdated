    ...
    try:
        ...
        for i, question in enumerate(request.questions):
            ...
            if result_primary.get("success", False) and result_primary.get("confidence_score", 0.0) >= 0.4:
                ...
                logger.info(f"[kg290] Confidence too low ({result_primary.get('confidence_score', 0.0)}), trying fallback model (meta-llama/llama-4-maverick-17b-128e-instruct)")
                result_fallback = await cloud_only_analysis(question, pdf_text, model_id="meta-llama/llama-4-maverick-17b-128e-instruct")
